...
func Test_initBundled_createsDirAndFiles(t *testing.T) {
	...
	names, err := NewCatalog(dir).list()
	require.NoError(t, err)
	assert.Equal(t, []string{"basic", "catppuccin-latte", "catppuccin-mocha", "dracula", "gruvbox", "nord", "solarized-dark", "yazi", "ydiff"}, names)
...
func Test_gallery(t *testing.T) {
	...
	for _, name := range []string{"basic", "catppuccin-latte", "catppuccin-mocha", "dracula", "gruvbox", "nord", "solarized-dark", "ydiff", "yazi"} {
		th, ok := gallery[name]
